...
```sh
make docker-build docker-push IMG=<some-registry>/customer-scaler:tag
```
...
```sh
make deploy IMG=<some-registry>/customer-scaler:tag
```
...
```sh
make build-installer IMG=<some-registry>/customer-scaler:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/customer-scaler/<tag or branch>/dist/install.yaml
```
